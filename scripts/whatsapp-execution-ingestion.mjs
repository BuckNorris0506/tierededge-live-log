...
import {
  createWhatsappExecutionPreview,
  extractImagePathsFromMessageText,
  handleWhatsappExecutionCommand,
} from './execution-screenshot-utils.mjs';

function usage() {
  console.error('Usage:');
  console.error('  node scripts/whatsapp-execution-ingestion.mjs preview --sender <sender> --image <path> [more-paths...]');
  console.error('  node scripts/whatsapp-execution-ingestion.mjs preview --sender <sender> --message-file <path>');
  console.error('  node scripts/whatsapp-execution-ingestion.mjs command --sender <sender> --command "CONFIRM ALL" [--rebuild]');
}
...
async function main() {
  ...
  if (args.mode === 'preview') {
    ...
    if (!imagePaths.length) {
      throw new Error('no_image_attachments_found');
    }
    const previewEntry = createWhatsappExecutionPreview({
  ...
  if (args.mode === 'command') {
    ...
    const result = handleWhatsappExecutionCommand({
      senderKey: args.sender,
...
main().catch((error) => {
  console.error(`whatsapp-execution-ingestion failed: ${error.message}`);
  process.exit(1);
});
